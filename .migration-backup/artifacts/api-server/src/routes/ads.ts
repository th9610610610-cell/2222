...
router.get('/all', requireAuth, requireAdmin, async (_req, res) => {
  try {
    ...
    console.error('[ads] GET /all error:', e)
    res.status(500).json({ error: e.message || 'Failed to fetch ads' })
  }
})
...
router.post('/', requireAuth, requireAdmin, async (req: AuthRequest, res) => {
  try {
    ...
    const adType = validTypes.includes(type) ? type : 'text'
    const [ad] = await db.insert(adsTable).values({
      type: adType as 'text' | 'image' | 'video',
      title: title || '',
      content,
      link_url: link_url || '',
...
router.patch('/:id', requireAuth, requireAdmin, async (req: AuthRequest, res) => {
  try {
    ...
    console.error('[ads] PATCH error:', e)
...
router.delete('/:id', requireAuth, requireAdmin, async (req: AuthRequest, res) => {
  try {
    ...
    res.status(500).json({ error: e.message || 'Failed to delete ad' })
  }
})
